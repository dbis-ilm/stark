...
import dbis.stark.spatial.indexed.live.{LiveIntervalIndexedSpatialRDDFunctions, LiveIndexedSpatialRDDFunctions}
...
class PlainSpatialRDDFunctions[G <: STObject : ClassTag, V: ClassTag](
  ...
  def intersects2(o: G) = new SpatialFilterRDD[G,V](rdd, o, JoinPredicate.INTERSECTS)
  /**
  ...
  def intersects(qry: G) = rdd.mapPartitionsWithIndex({ (idx, iter) =>
    ...
    val partitionCheck = rdd.partitioner.map { p =>
      p match {
        ...
        case tp: TemporalPartitioner =>
          tp.partitionBounds(idx).start<=(qry.getTemp.get.end.get)
    ...
    if (partitionCheck)
      doWork(qry, iter, Predicates.intersects _)
    else
      Iterator.empty
  })
  def containedby2(o: G) = new SpatialFilterRDD[G,V](rdd, o, JoinPredicate.CONTAINEDBY)
  /**
  ...
  def containedby(qry: G) = rdd.mapPartitionsWithIndex({ (idx, iter) =>
    val partitionCheck = rdd.partitioner.map { p =>
      p match {
        ...
        case tp: TemporalPartitioner =>
          tp.partitionBounds(idx).intersects(qry.getTemp.get)
        case _ => true
      }
    }.getOrElse(true)

    if (partitionCheck)
      doWork(qry, iter, Predicates.containedby _)
    else
      Iterator.empty
  ...
  /**
    ...
    */
  def contains(qry: G): RDD[(G, V)] = rdd.mapPartitionsWithIndex({ (idx, iter) =>
    val partitionCheck = rdd.partitioner.map { p =>
      p match {
        case sp: SpatialPartitioner =>
          Utils.toEnvelope(sp.partitionBounds(idx).extent).intersects(qry.getGeo.getEnvelopeInternal)
        case tp: TemporalPartitioner =>
          // der start der partition muss vor dem start vom query liegen
          tp.partitionBounds(idx).start<=(qry.getTemp.get.start)
        case _ =>
          true
      }
    }.getOrElse(true)
    ...
    if (partitionCheck) {
      ...
      doWork(qry, iter, Predicates.contains _)
    }
    else {
      //println("do nothing")
      Iterator.empty
  ...
  def doWork[G <: STObject : ClassTag, V: ClassTag](
                                                     qry: G,
                                                     iter: Iterator[(G, V)],
                                                     predicate: (STObject, STObject) => Boolean
                                                   ) = iter.filter { case (g, _) =>
  ...
  def join[V2 : ClassTag](other: RDD[(G, V2)], pred: JoinPredicate, partitioner: Option[SpatialPartitioner] = None): RDD[(V,V2)] =
    new JoinSpatialRDD(
