...
class LocalDBScanSpec extends FlatSpec with Matchers {
  ...
  "LocalDBScan" should "expand a 2D cluster by points from a list" in {
    ...
    val points = List(
      ClusterPoint(Vectors.dense(178.505256857926,13.0232102015073)),
      ClusterPoint(Vectors.dense(192.647916294684,-54.9397894514337)),
      ClusterPoint(Vectors.dense(221.447056375756,54.7299660721257)),
      ClusterPoint(Vectors.dense(27.3622293956578,-1401.0191693902)),
      ClusterPoint(Vectors.dense(88.692161857309,59.1680534628075)),
      ClusterPoint(Vectors.dense(-10.9376996678452,57.462273892661)),
      ClusterPoint(Vectors.dense(1125.10539018549,-99.3853457272053)),
      ClusterPoint(Vectors.dense(52.7498085418551,-9.04225181250282)),
      ClusterPoint(Vectors.dense(57.0464906188891,63.3980855359247)),
      ClusterPoint(Vectors.dense(9.09417446372015,-140.061523108837))
    )
    val start = ClusterPoint(Vectors.dense(52.7498085418551,-9.04225181250282))
  ...
  it should "not expand a cluster for a noise point" in {
    ...
    val start = ClusterPoint(Vectors.dense(27.3622293956578,-1401.01916939022))
    val res = dbscan.expandCluster(start, 42, points)
  ...
  it should "update cluster ids of points using a mapping table" in {
    ...
    dbscan.mapClusterId(ClusterPoint(Vectors.zeros(2), 2, ClusterLabel.Core, None, true), map).clusterId should be (10)
    dbscan.mapClusterId(ClusterPoint(Vectors.zeros(2), 4, ClusterLabel.Core, None, true), map).clusterId should be (10)
    dbscan.mapClusterId(ClusterPoint(Vectors.zeros(2), 6, ClusterLabel.Core, None, true), map).clusterId should be (20)
    dbscan.mapClusterId(ClusterPoint(Vectors.zeros(2), 1, ClusterLabel.Core, None, true), map).clusterId should be (1)
  }
